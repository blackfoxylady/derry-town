...
function select(id,focus){const s=all.find(s=>s.id===id);if(!s)return;state.selected=id;q('#intro').hidden=true;const panel=q('#detail');panel.hidden=false;const conf={A:'A · Text-anchored relationship',B:'B · Inferred position',C:'C · Proposed location',U:'U · Unlocated / off map'};
 ...
 panel.innerHTML=`<div class="detail-top"><div><div class="eyebrow">Place ${esc(String(s.id).padStart(2,'0'))}</div><h2>${esc(s.name)}</h2></div><button class="close" id="closeDetail" aria-label="Close place">×</button></div><div class="meta">${esc(s.period)}</div><p><span class="badge">${conf[s.confidence]}</span></p><p>${esc(s.note)}</p><details open><summary>Sources & evidence</summary><ul class="refs">${refs.length?refs.map(g=>`<li>${esc(g.reference)}${g.notes.map(n=>'<br>'+esc(n)).join('')}</li>`).join(''):`<li>${esc(s.reference)}</li>`}</ul></details>`;
