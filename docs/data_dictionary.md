...
| `url` | varchar(2000) | нет | HTTP(S)-ссылка; у EPUB пусто. |
| `role` | text | нет | Для чего использован источник. |
| `metadata` | jsonb | нет | Дополнительные данные происхождения, исходный индекс, хеш источника. |

## `atlas_evidence` — Evidence

Связь объекта с источником, главой/параграфом и пояснением.
...
| `paragraph` | integer ≥ 0 | да | Номер параграфа данной EPUB-экстракции, не страница. Null для других ссылок. |
