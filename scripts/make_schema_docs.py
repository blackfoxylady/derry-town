...
models=list(TABLES.values())+[m.Revision,m.MapState,m.Character,m.Tag,m.Photo,m.Photo.characters.through,m.Photo.tags.through,m.PlaceCover]
...
'Evidence':'Связь объекта с источником, главой и пояснением.',
...
for model in models:
 ...
 for f in model._meta.fields:
  ...
  if f.is_relation:
   ...
   if name=='PlaceCover' and f.name=='year':desc='Год эпохи романа, обязателен; уникален в пределах места (cover_unique_feature_year).'
   if name=='PlaceCover' and f.name=='alt':desc='Обязательный alt-текст для доступности; подписи на экране нет.'
   if name=='PlaceCover' and f.name=='alt_ru':desc='Русский alt; пустой откатывается на английский.'
   if name=='PlaceCover' and f.name=='created':desc='UTC время загрузки.'
...
boxes={
 ...
 'Source':(30,480,['PK key','title · url · role']), 'Evidence':(440,480,['PK key','FK feature_id','FK source_id','reference · note']),
 ...
 'PlaceCover':(860,1000,['PK id','sha256 · ext · alt · alt_ru','feature_key (строка, не FK)','year · width · height'])}
W,H=280,140
